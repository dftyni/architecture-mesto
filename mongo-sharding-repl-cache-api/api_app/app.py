...
# Consul API endpoint
consul_url = os.environ["CONSUL_SRV"] + "/v1/agent/service/register"

service_address = os.getenv("SERVICE_ADDR", "127.0.0.1")
...
payload = {
    ...
    "Port": 8080,
    "Check": {
        "HTTP": f"http://{service_address}:8080/health",
        "Interval": "10s",  # Health check interval
        "Timeout": "1s",  # Health check timeout
        "DeregisterCriticalServiceAfter": "30s"  # Deregister if unhealthy
    }
}
...
async def startup():
    ...
    if response.status_code == 200:
        logger.log(1, f"Service successfully registered in Consul!")
    else:
        logger.log(1, f"Failed to register service. Status code: {response.status_code}, Response: {response.text}")
